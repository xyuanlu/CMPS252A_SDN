...
class l3_switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    if dpid not in self.arpTable:
      ...
      for fake in self.fakeways:
        self.arpTable[dpid][IPAddr(fake)] = Entry(of.OFPP_NONE,
         dpid_to_mac(dpid))
        log.warning("fakeway: %s", self.fakeways)
    ...
    if isinstance(packet.next, ipv4):
      ...
      log.debug("dstaddr :%s dpid %d",dstaddr,dpid)
      if dstaddr in self.arpTable[dpid]:
        # We have info about what port to send it out on...
        prt = self.arpTable[dpid][dstaddr].port
        mac = self.arpTable[dpid][dstaddr].mac
        log.debug("dstaddr in self.arpTable[dpid] prt:%d, mac:%s", prt,mac)
        if prt == inport:
      ...
      a = packet.next
      log.debug("arp packets, dstaddr :%s dpid %d",packet.next.protodst ,dpid)
          ...
          if a.protosrc != 0:
            ...
            if a.opcode == arp.REQUEST:
              ...
              if a.protodst in self.arpTable[dpid]:
                # We have an answer...
                log.debug("in arp table poar:%d mac:%s ", self.arpTable[dpid][a.protodst].port, self.arpTable[dpid][a.protodst].mac )
                ...
                if not self.arpTable[dpid][a.protodst].isExpired():
                  ...
                  e = ethernet(type=packet.type, src=self.arpTable[dpid][a.protodst].mac,
                               dst=a.hwsrc)
                  e.set_payload(r)
